...
function naughtyList(arr, habit){
    naughties = []
    for(let i=0; i<arr.length; i++){
        for(j in arr[i].habits){
            if (arr[i].habits[j] === habit){
                naughties.push(`${arr[i].firstName} ${arr[i].lastName}`)
            }
        }
    }
    console.log(naughties);
    return naughties;
}

naughtyList(students, badHabit3);